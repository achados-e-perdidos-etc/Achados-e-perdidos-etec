...
app = Flask(__name__)
# Permitir conexões externas (Desktop e Web)
CORS(app)
...
@app.route('/')
def home():
    return jsonify({"status": "API Achados e Perdidos ETEC Funcionando!", "versao": "1.0"})
...
def get_itens():
    ...
    for row in rows:
        itens.append({
            ...
            "foto": row[5] or "",
            "status": row[6],
            "solicitado_por": row[7],
            "rm_aluno": row[8]
        })
    return jsonify(itens)

# Rota para a Secretaria cadastrar novo item (do Desktop)
...
def cadastrar_item():
    ...
    return jsonify({"success": True, "message": "Objeto cadastrado na nuvem com sucesso!"})

# Rota para o Aluno solicitar coleta (da Web)
...
if __name__ == "__main__":
    # Render atribui uma porta dinâmica na variável PORT
