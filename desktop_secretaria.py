...
class AdminDesktopApp:
  ...
  def __init__(self, root):
    ...
    style.configure(
        "TLabel", background="#1e1e2e", foreground="#ffffff", font=("Helvetica", 10)
    )
    ...
    lbl_title = tk.Label(
        header,
        text="SECRETARIA - ACHADOS E PERDIDOS",
    ...
    tk.Label(
        self.form_frame,
        text="Descrição do Item (txt_descricao):",
        bg="#1e1e2e",
        fg="#e2e8f0",
    ...
    # Categoria
    tk.Label(
        self.form_frame, text="Categoria:", bg="#1e1e2e", fg="#e2e8f0"
    ).grid(row=2, column=0, sticky="w")
    self.cb_categoria = ttk.Combobox(
        self.form_frame,
        values=["MOCHILA", "ROUPAS", "ACESSÓRIOS", "ESCOLARES", "OUTROS"],
        state="readonly",
        width=30,
    )
    ...
    tk.Label(
        self.form_frame,
        text="Local Encontrado (txt_local):",
        bg="#1e1e2e",
        fg="#e2e8f0",
    ).grid(row=6, column=0, sticky="w")
    self.txt_local = tk.Entry(
        self.form_frame,
        width=32,
        font=("Helvetica", 10),
        bg="#334155",
    ...
    tk.Label(
        self.form_frame, text="Status do Item:", bg="#1e1e2e", fg="#e2e8f0"
    ).grid(row=8, column=0, sticky="w")
    self.cb_status = ttk.Combobox(
        self.form_frame,
        values=["Disponível", "Solicitado", "Entregue"],
        state="readonly",
        width=30,
    )
    ...
    # Foto
    tk.Label(
        self.form_frame, text="Foto do Objeto:", bg="#1e1e2e", fg="#e2e8f0"
    ).grid(row=10, column=0, sticky="w")
    btn_foto = tk.Button(
        self.form_frame,
        text="📷 Selecionar Foto...",
        command=self.carregar_foto,
        bg="#0284c7",
        fg="white",
    ...
    # Botões do Formulário (Correção do fill="x" para sticky="ew")
    ...
    self.btn_salvar.grid(
        row=12, column=0, columnspan=2, sticky="ew", pady=(10, 5)
    )
    ...
    table_frame = tk.LabelFrame(
        container,
        text=" Lista de Itens Cadastrados ",
        bg="#1e1e2e",
        fg="#38bdf8",
        font=("Helvetica", 10, "bold"),
        padx=10,
  ...
  def carregar_para_edicao(self):
    ...
    if not selected_item:
      messagebox.showwarning(
          "Seleção", "Por favor, selecione um item na tabela para editar!"
      )
      return
  ...
  def salvar_item(self):
    ...
    try:
      if self.item_id_em_edicao:
        # EDITAR (PUT)
        res = requests.put(
            f"{API_URL}/api/itens/{self.item_id_em_edicao}", json=payload
        )
  ...
  def excluir_item(self):
    ...
    if not selected_item:
      messagebox.showwarning(
          "Seleção", "Por favor, selecione um item na tabela para excluir!"
      )
      return
    ...
    try:
      ...
      if res.status_code == 200:
        ...
        for i in itens:
          ...
          aluno_info = (
              f"{i['solicitado_por']} (RM: {i['rm_aluno']})"
              if i["solicitado_por"]
              else "-"
          )
          self.tree.insert(
