...
class AdminDesktopApp:
    def __init__(self, root):
        ...
        btn_excluir = tk.Button(table_frame, text="🗑 Excluir Item Selecionado", command=self.excluir_item, bg="#dc2626", fg="white", font=("Helvetica", 9, "bold"), relief="flat", cursor="hand2")
        btn_excluir.pack(fill="x", pady=(5, 0))
    ...
    def excluir_item(self):
        ...
        if messagebox.askyesno("Confirmar Exclusão", f"Tem certeza que deseja excluir o item ID #{item_id}?"):
            try:
                res = requests.delete(f"{API_URL}/api/itens/{item_id}", timeout=15)
                if res.status_code == 200:
                    messagebox.showinfo("Sucesso", "Item excluído com sucesso!")
                    self.carregar_tabela()
                else:
                    messagebox.showerror("Erro", f"Falha ao excluir item (Código: {res.status_code}).")
            except Exception as e:
                messagebox.showerror("Erro de Conexão", f"Não foi possível conectar ao servidor remoto:\n{e}")
